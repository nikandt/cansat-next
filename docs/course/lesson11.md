...
The barometer and IMU are on the same I2C bus as the extension header. Check the addresses of those devices at the page [On-Board sensors](../CanSat-hardware/on_board_sensors#IMU). Similar to SPI, you can use these pins for connecting other I2C devices, but if they are used as GPIO pins, the IMU and barometer are disabled.
